...
feeds/*.yml (168 源)
    ↓
Source 层(RSSSource + build_source 工厂,并发抓取,抓取窗口内全量)
...
| 组件 | 路径 | 职责 |
|---|---|---|
| Source 层 | `src/sources/` | RSS 抓取,RSSHub 路由,抓取窗口内全量,Source Protocol |
...
2. `_fetch_all_items` 用 `asyncio.gather` 并发抓取所有 enabled 源(抓取窗口内全量),DedupStore 过滤已处理项,返回 `list[ContentItem]`
