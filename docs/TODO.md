...
本批重构:砍掉 Tier3 深度 agent 分析,管线简化为两段式(抓取 → Tier1 分类+打分+摘要 → Tier2 选取+去重 → 渲染日报);批量主题去重(大组分块并发);删除每源 30 截断,DedupStore 跨轮去重保证每条目恰好处理一次(无丢弃无重处理);新增 video 分类与 168 源(参考 rss-feed)。
